...
export default function AdminLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error || t('adminlogin.errorInvalidCredentials'));
        setLoading(false);
        return;
      }
      ...
      if (!sess.success || !sess.role) {
        setError(sess.error || t('adminlogin.errorCompleteSignIn'));
        setLoading(false);
        return;
      }
      ...
      setError(t('adminlogin.errorConnection'));
      setLoading(false);
    }
  }
    ...
    <div className="py-24">
      <div className="max-w-md mx-auto px-4">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold">{t('adminlogin.heroTitle')}</h1>
          <p className="text-muted-foreground">{t('adminlogin.heroSubtitle')}</p>
        </div>
        <Card>
          <CardContent className="p-6">
            ...
            <button
              onClick={async () => {
                ...
                if (!result.success) {
                  setError(result.error || t('adminlogin.errorGoogleSignIn'));
                  setLoading(false);
                }
              }}
              disabled={loading}
            ...
            <div className="relative mb-4">
              ...
              <div className="relative flex justify-center text-xs"><span className="bg-white px-2 text-gray-400">{t('adminlogin.orSignInWithEmail')}</span></div>
            </div>
            ...
            <div className="mt-4 text-center text-sm text-muted-foreground">
              <Link href="/login" className="text-christina-blue hover:underline">{t('adminlogin.parentLoginLink')}</Link>
            </div>
          </CardContent>
        </Card>
      </div>
