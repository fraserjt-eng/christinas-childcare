...
// "Enter a day" — back-office bulk attendance entry. Staff key a paper sign-in
// sheet straight into the system: pick a date (and center, if a cross-center
// director), then type each child's in/out time and who dropped off / picked up.
// Saves the same attendance rows a kiosk tap writes, so the day flows into the
// Attendance Hub dashboard and the DCYF export with no transcription. Reads and
// writes go through the session-gated, service-role /api/admin/attendance/day.
...
import { ChevronLeft, ChevronRight, Save, Loader2, CalendarDays, ArrowLeft, Users } from 'lucide-react';
...
export default function AttendanceEntryPage() {
      ...
      for (const room of payload.rooms ?? []) {
        for (const ch of room.children) {
          next[ch.id] = {
            ...
            signedInBy: ch.signedInBy,
            signedOutBy: ch.signedOutBy,
            absent: false,
          };
        }
      }
  ...
  async function save() {
    ...
    try {
      ...
      line += '.';
      if (Array.isArray(out.problems) && out.problems.length) {
        line += ` Needs a look: ${out.problems.join('; ')}`;
      }
      setMsg(line);
      load(date, center); // refresh so prefill reflects what's now stored
    } catch {
  ...
  return (
    <div className="container mx-auto max-w-5xl space-y-5 p-4 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <CalendarDays className="h-8 w-8 text-christina-red" />
          <div>
            <h1 className="text-2xl font-bold md:text-3xl">Enter a day</h1>
            <p className="text-muted-foreground">
              Key a paper sign-in sheet into the system{data?.centerName ? ` for ${data.centerName}` : ''}. It flows straight into the dashboard and the state export.
            </p>
          </div>
        </div>
        <Link href="/admin/attendance/hub" className="inline-flex items-center gap-1.5 text-sm font-medium text-christina-red hover:underline">
      ...
      {error && <p className="text-sm font-medium text-christina-red">{error}</p>}
        ...
        <>
          <div className="space-y-5">
            {data.rooms.map((room) => (
              <Card key={room.room}>
                <CardHeader>
                  <CardTitle className="text-lg">{room.room} <span className="text-sm font-normal text-muted-foreground">({room.children.length})</span></CardTitle>
                </CardHeader>
                  ...
                  <table className="w-full min-w-[640px] text-left text-sm">
                    ...
                    <tbody>
                      {room.children.map((ch) => {
                        ...
                        const listId = `parents-${ch.id}`;
                        return (
                          <tr key={ch.id} className={'border-b last:border-0 ' + (e.absent ? 'opacity-50' : '')}>
                            <td className="py-2 pr-2 font-medium">{ch.name}</td>
                            <td className="py-2 px-2">
                              <input type="time" value={e.arrival} disabled={e.absent}
                                onChange={(ev) => setField(ch.id, 'arrival', ev.target.value)}
                                className="rounded border px-1.5 py-1 text-sm" />
                            </td>
                            <td className="py-2 px-2">
                              <input type="time" value={e.departure} disabled={e.absent}
                                onChange={(ev) => setField(ch.id, 'departure', ev.target.value)}
                                className="rounded border px-1.5 py-1 text-sm" />
                            </td>
                            <td className="py-2 px-2">
                              <input list={listId} value={e.signedInBy} disabled={e.absent}
                                onChange={(ev) => setField(ch.id, 'signedInBy', ev.target.value)}
                                placeholder="name"
                                className="w-32 rounded border px-1.5 py-1 text-sm" />
                            </td>
                            <td className="py-2 px-2">
                              <input list={listId} value={e.signedOutBy} disabled={e.absent}
                                onChange={(ev) => setField(ch.id, 'signedOutBy', ev.target.value)}
                                placeholder="name"
                                className="w-32 rounded border px-1.5 py-1 text-sm" />
                            </td>
