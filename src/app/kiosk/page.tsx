...
async function lookupFamilyByPin(pin: string): Promise<KioskFamily | null> {
  // Find family by PIN
...
async function getTodayAttendance(childId: string): Promise<AttendanceRow | null> {
...
function PinScreen({ onSuccess }: { onSuccess: (family: KioskFamily) => void }) {
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 gap-8">
          ...
          {Array.from({ length: MAX_PIN }).map((_, i) => (
            <div
              ...
              className={`w-6 h-6 rounded-full border-2 transition-all duration-150 ${
                i < pin.length ? 'bg-christina-red border-christina-red scale-110' : 'bg-white border-gray-300'
              }`}
            />
          ))}
        </div>

        {error && <p className="text-red-600 font-semibold text-xl -mt-4">{error}</p>}
        ...
        <div className="grid grid-cols-3 gap-4 w-full max-w-xs">
          {buttons.map((btn, idx) => {
            ...
            if (btn === 'back') {
              return (
                <button key="back" onClick={handleBackspace} disabled={loading || pin.length === 0}
            ...
            return (
              <button key={btn} onClick={() => handleDigit(btn)} disabled={loading || pin.length >= MAX_PIN}
        ...
        {loading && <p className="text-gray-400 text-lg animate-pulse">Checking PIN...</p>}
      </div>
...
// ============================================================================
// Child Card (reads attendance from Supabase)
// ============================================================================

function ChildCard({ child, familyId, onAction }: { child: FamilyChildRow; familyId: string; onAction: () => void }) {
  const [attendance, setAttendance] = useState<AttendanceRow | null>(null);
  const [busy, setBusy] = useState(false);
  const [loaded, setLoaded] = useState(false);

  useEffect(() => {
    getTodayAttendance(child.id).then((rec) => {
      setAttendance(rec);
      setLoaded(true);
    });
  }, [child.id]);

  const isCheckedIn = attendance !== null && attendance.check_in !== null && attendance.check_out === null;

  async function handleToggle() {
    setBusy(true);
    if (isCheckedIn) {
      await checkOutChild(child.id);
    } else {
      await checkInChild(child, familyId);
    }
    const updated = await getTodayAttendance(child.id);
    setAttendance(updated);
    setBusy(false);
    onAction();
  }

  if (!loaded) return <div className="bg-white rounded-2xl border-2 border-gray-100 shadow-sm p-5 animate-pulse h-24" />;
    ...
    <div className="bg-white rounded-2xl border-2 border-gray-100 shadow-sm p-5 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          {child.date_of_birth && <span className="text-sm text-gray-500">{getChildAge(child.date_of_birth)}</span>}
          {child.classroom && (
            <>
              <span className="text-gray-300">|</span>
              <span className="text-sm text-gray-500 capitalize">{child.classroom}</span>
            </>
          )}
        </div>
...
function WelcomeScreen({ family, onDone }: { family: KioskFamily; onDone: () => void }) {
  ...
  return (
    <div className="flex flex-col min-h-screen" onPointerDown={resetTimer} onKeyDown={resetTimer}>
      <div className="bg-christina-red text-white px-6 py-5 flex items-center gap-4">
        <ZLogo size={56} />
        <div className="flex-1">
          <div className="text-2xl font-bold leading-tight">Welcome, {lastName} family!</div>
          <div className="text-base opacity-90 leading-tight">Christina&apos;s Child Care Center</div>
        </div>
      ...
      <div className="flex-1 px-5 py-6 flex flex-col gap-4 overflow-auto">
        <p className="text-gray-500 text-base px-1">Tap Check In or Check Out for each child below.</p>
        {family.children.length === 0 ? (
          <div className="text-center py-12 text-gray-400 text-lg">No children on this account yet.</div>
        ) : (
          family.children.map((child) => (
            <ChildCard key={child.id} child={child} familyId={family.id} onAction={resetTimer} />
          ))
        )}
      </div>
...
export default function KioskPage() {
  ...
  return activeFamily
    ? <WelcomeScreen family={activeFamily} onDone={() => setActiveFamily(null)} />
    : <PinScreen onSuccess={(f) => setActiveFamily(f)} />;
}
